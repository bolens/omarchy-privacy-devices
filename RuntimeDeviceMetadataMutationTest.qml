...
ShellRoot {
  ...
  Timer {
    ...
    running: true
    onTriggered: {
      if (root.commits.length !== 1) throw new Error("rapid device metadata changes were not coalesced")
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (settings.deviceLabels["alsa_input.desk"] !== undefined || settings.itemLabels.microphone !== undefined
